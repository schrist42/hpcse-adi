                          ...
                          bool&   benchmark)
{
	...
	// parse options without value -> could also be done in add_options
...
int main(int argc, char* argv[])
{
	...
	bool result = process_command_line(argc, argv, N, L, dt, Du, Dv, F, k, nSteps, visualize, pngname, benchmark);
	if (!result)
	    return 1;
    ...
    if (visualize) {
        ...
        viewer.visualize(simulation); // will run, not benchmark the simulation
    }
    else {
        if (benchmark) {
