...
# load data
data1 = np.loadtxt('omp_size_scaling_8.dat') # threads, time, size, error
data2 = np.loadtxt('mpi_data_32_size.dat') # tasks, time, size, error
data3 = np.loadtxt('hybrid_size_data.dat') # ntasks, nthreads, time, size, err
data4 = np.loadtxt('serial_novec.dat') # time, size, err

fig1, ax1 = plt.subplots()

if len(data1[0]) > 2: # with error
    ax1.errorbar(data1[:,2], data1[:,1], yerr=data1[:,3], label='OpenMP version, 8 threads', color="r")
    ax1.errorbar(data2[:,2], data2[:,1], yerr=data2[:,3], label='MPI version, 32 processes', color="b")
    ax1.errorbar(data3[:,3], data3[:,2], yerr=data3[:,4], label='Hybrid version, 32 processes, 8 threads, transpose with data type', color="g")
    ax1.errorbar(data4[:,1], data4[:,0], yerr=data4[:,2], label='Serial - Non vectorized',color="k")
else: # without error
...
ax1.set_xscale('log')
ax1.set_yscale('log')
ax1.set_xlabel('System size (in one grid dimension)')
ax1.set_ylabel('Time per step [s]')
ax1.set_title('Comparison of all implementations - system size scaling')
