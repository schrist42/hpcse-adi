...
# load data
data_lt = np.loadtxt('mpi_lt_32_size.dat') # tasks, time, size, error
data_datatype = np.loadtxt('mpi_data_32_size.dat') # tasks, time, size, error

fig1, ax1 = plt.subplots()

if len(data_lt[0]) > 3: # with error
    ax1.errorbar(data_lt[:,2], data_lt[:,1], yerr=data_lt[:,3], label='local transpose', color='r')
    ax1.errorbar(data_datatype[:,2], data_datatype[:,1], yerr=data_datatype[:,3], label='different receive data type', color='b')
else: # without error
...
ax1.set_title('Comparison of two transpose approaches')
...
plt.savefig('mpi_transpose_32.pdf')
plt.show()
