...
function jobfile {
	echo -e "#!/bin/bash -l\n\n#SBATCH --ntasks=1\n#SBATCH --time=$3\n\necho \"executes on nodes:\"" > serial_vec_$1.job
	echo -e 'echo $SLUM_JOB_NODELIST' >> serial_vec_$1.job
	echo -e "\necho \"output (if any follows:)\"\n\nif [ ! -z \$1 ]; then\n\tif [ ! -d \"\$1\" ]; then\n\t\tmkdir \"\$1\"\n\tfi\nfi \n\naprun -B /users/stud06/hpcse-adi/serial/src_daint/main_vectorize -N $1 --dt $dt --nsteps $steps --benchmark" >> serial_vec_$1.job
}
