...
function jobfile {
    jobfilename=serial_novec_$1.job
	echo -e "#!/bin/bash -l\n\n#SBATCH --ntasks=1\n#SBATCH --time=$3\n\necho \"executes on nodes:\"" > $jobfilename
	echo -e 'echo $SLUM_JOB_NODELIST' >> $jobfilename
	echo -e "\necho \"output (if any follows:)\"\n\nif [ ! -z \$1 ]; then\n\tif [ ! -d \"\$1\" ]; then\n\t\tmkdir \"\$1\"\n\tfi\nfi \n\naprun -B /users/stud06/hpcse-adi/serial/src_daint/main_novectorize -N $1 --dt $dt --nsteps $steps --benchmark" >> $jobfilename
}
...
do
    ...
    echo "sbatch serial_novec_$N.job"
    sbatch serial_novec_$N.job
done
